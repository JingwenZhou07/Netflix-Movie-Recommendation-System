...
class Dense(Layer):
    ...
    def _call(self, inputs):
        x_u = inputs[0]
        x_u = tf.nn.dropout(x_u, 0.2 - self.dropout)
        x_u = tf.matmul(x_u, self.vars['weights_u'])

        x_v = inputs[1]
        x_v = tf.nn.dropout(x_v, 0.2 - self.dropout)
...
class StackGCN(Layer):
    ...
    def __init__(self, input_dim, output_dim, support, support_t, num_support, u_features_nonzero=None,
        ...
        self.support = tf.compat.v1.sparse_split(axis=1, num_split=num_support, sp_input=support)
        self.support_transpose = tf.compat.v1.sparse_split(axis=1, num_split=num_support, sp_input=support_t)
...
class OrdinalMixtureGCN(Layer):
    ...
    def __init__(self, input_dim, output_dim, support, support_t, num_support, u_features_nonzero=None,
        ...
        self.bias = bias
        support = tf.compat.v1.sparse_split(axis=1, num_split=num_support, sp_input=support)

        support_t = tf.compat.v1.sparse_split(axis=1, num_split=num_support, sp_input=support_t)
    ...
    def _call(self, inputs):
        ...
        if self.sparse_inputs:
            ...
            x_u = tf.nn.dropout(inputs[0], 0.2 - self.dropout)
            x_v = tf.nn.dropout(inputs[1], 0.2 - self.dropout)
...
class BilinearMixture(Layer):
    ...
    def _call(self, inputs):
        ...
        u_inputs = tf.nn.dropout(inputs[0], 0.2 - self.dropout)
        v_inputs = tf.nn.dropout(inputs[1], 0.2 - self.dropout)
